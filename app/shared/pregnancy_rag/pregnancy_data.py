...
class PregnancyDataService:
    ...
    def _initialize_data(self) -> Dict[int, PregnancyWeek]:
        """Initialize pregnancy week data with key developments (all 40 weeks)"""
        # Use full 40-week data from pregnancy_data_full.py
        from app.shared.pregnancy_rag.pregnancy_data_full import get_all_40_weeks_data
        return get_all_40_weeks_data()
        
        # Legacy code below (commented out - now using full dataset)
        '''
        data = {}
    ...
    def get_all_weeks(self) -> Dict[int, PregnancyWeek]:
        ...
        if self.use_qdrant and self.qdrant_service:
            return self.qdrant_service.get_all_weeks()
        else:
            return self.pregnancy_data
    
    def get_weeks_by_trimester(self, trimester: int) -> Dict[int, PregnancyWeek]:
        """Get all weeks for a specific trimester"""
        if self.use_qdrant and self.qdrant_service:
            weeks_list = self.qdrant_service.get_weeks_by_trimester(trimester)
            return {week.week: week for week in weeks_list}
        else:
            return {
                week: data for week, data in self.pregnancy_data.items() 
    ...
    def semantic_search(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        Only available when using Qdrant
        """
        if not self.use_qdrant or not self.qdrant_service:
            raise ValueError("Semantic search is only available when using Qdrant storage")
        
        return self.qdrant_service.semantic_search(query, limit)
